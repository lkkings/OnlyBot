...
from onlybot.exceptions.file_exceptions import (
    FileNotFound,
    FilePermissionError,
)
from onlybot.utils.common import get_resource_path
...
class ConfigManager:
    ...
    def __init__(self, filepath: str = onlybot.ONLY_BOT_CONFIG_FILE_PATH):
    ...
    def generate_config(self, app_name: str, save_path: Path):
        ...
        default_config = (
            yaml.safe_load(
                Path(get_resource_path(onlybot.onlybot_DEFAULTS_FILE_PATH)).read_text(
                    encoding="utf-8"
                )
            )
            or {}
        ...
        if app_name in default_config:
            ...
            logger.info(
                "{0} 应用配置文件生成成功，保存至 {1}".format(app_name, save_path)
            )
        else:
            logger.info("{0} 应用配置未找到".format(app_name))
    ...
    def update_config_with_args(self, app_name: str, **kwargs):
        ...
        if click.confirm(
            f"是否要使用命令行的参数更新配置文件？{Path.cwd() / self.filepath}",
            default=True,
        ):
            # 备份原始配置文件
            self.backup_config()
...
class TestConfigManager:
    ...
    def get_test_config(cls, app_name: str) -> dict:
        return ConfigManager(onlybot.TEST_CONFIG_FILE_PATH).get_config(app_name)
